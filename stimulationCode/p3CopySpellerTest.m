...
% Timing information
%--------------------------------------------------------------------------
pauseBeforeCueInSec         = 1;
cueDurationInSec            = 2;
pauseAfterCueInSec          = 3;
pauseBetweenRoundsInSec     = pauseBeforeCueInSec + cueDurationInSec + pauseAfterCueInSec;
...
if readStenPerf
    renderTime  = cell(nChar, 2);
    flipTime    = cell(nChar, 2);
end
...
st1.sc              = generateScenarioCopySpellTest( stimuli, nChar );
st1.sc.texturesDir  = texturesDir;

% Top string image
%--------------------------------------------------------------------------
charList    = zeros(1, nChar);
for iChar = 1:nChar
    charList(iChar) = stimuli.stringSymbols.code( ismember( stimuli.stringSymbols.str, stringWord(iChar) ) );
end

hSize       = nChar*( max( cellfun(@(x) max(size(x,2)), stimuli.dimmed_symbols(:)) ) + stringGapInPixels );
...
resizeStringFactor  = stringHeighInPixels / vSize;  % CAN BE USEFUL FOR RESIZING GAP OF FEEDBACK STRING
...
%==========================================================================
nMaxEvents  = round( 2 * nChar *( 2 ...                             % start and stop events
...
keyPressed              = false;
detectedSymbolCode      = zeros( nChar, 1 );
stimuliStateSeqOnsets   = zeros( stimuli.number*nRepetitions, nChar );
for iRound = 1:nChar
    ...
    % Top string stimulus
    st1.sc.stimuli(iTopStrStim).stateSequence = [1 2 3];
    st1.sc.stimuli(iTopStrStim).durationSequenceInSec = [pauseBeforeCueInSec cueDurationInSec sum(durationSeq)];
...
%     winnerSymbol.iRowCol = [sRow, sCol];
    ...
    % Top string stimulus
    st1.sc.stimuli(iTopStrStim).stateSequence          = 3;
    ...
    % Feedback string stimuli
    iStrSymbUpdate = min( iRound, nChar );
    for iStrSymb = 1:nChar
