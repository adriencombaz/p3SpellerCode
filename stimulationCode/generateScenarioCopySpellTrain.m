function sc1 = generateScenarioCopySpellTrain( stimuli )

sc1 = [];
sc1.description         = 'P300 speller';
sc1.desired.scr.nCols   = stimuli.scr_cols;
sc1.desired.scr.nRows   = stimuli.scr_rows;
logThis( 'sc1.desired.scr.nCols = %d', sc1.desired.scr.nCols );
logThis( 'sc1.desired.scr.nRows = %d', sc1.desired.scr.nRows );
...
% feedback string: 
sc1.textures(iTex).filename = sprintf('fbString.png');
iTex = iTex + 1;
...
%--------------------------------------------------------------------------
% for iS = 1:numel(cueList)
    iStim                                       = iStim + 1;
    sc1.stimuli(iStim).description              = 'Feedback string';
    sc1.stimuli(iStim).stateSequence            = 1;
    sc1.stimuli(iStim).durationSequenceInSec    = Inf;
    sc1.stimuli(iStim).desired.position         = [0 0 0 0];
    sc1.stimuli(iStim).states(1).views.iTexture = stimuli.number + 4; % default
    sc1.stimuli(iStim).states(1).position       = [0 0 0 0];
    sc1.stimuli(iStim).states(2).views.iTexture = stimuli.number + 4; % fake end update
    sc1.stimuli(iStim).states(2).position       = [0 0 0 0];
    sc1.stimuli(iStim).states(3).views.iTexture = stimuli.number + 4; % update
    sc1.stimuli(iStim).states(3).position       = [0 0 0 0];
    sc1.stimuli(iStim).eventMatrix              = zeros(3);
    sc1.stimuli(iStim).eventMatrix(1, 3)        = find( cellfun( @(x) strcmp(x, 'Feedback string on'), {sc1.events(:).desc} ) );
    sc1.stimuli(iStim).eventMatrix(3, 2)        = find( cellfun( @(x) strcmp(x, 'Feedback string off'), {sc1.events(:).desc} ) );
% end
...
sc1.stimuli(iStim).desired.position         = [0 0 0 0];
sc1.stimuli(iStim).states(1).position       = [0 0 0 0];
sc1.stimuli(iStim).states(1).views.iTexture = stimuli.number + 5;
sc1.stimuli(iStim).states(2).position       = [0 0 0 0];
sc1.stimuli(iStim).states(2).views.iTexture = 0;
sc1.stimuli(iStim).eventMatrix              = zeros(2);
% sc1.stimuli(iStim).eventMatrix(2, 1)        = find( cellfun( @(x) strcmp(x, 'Feedback on'), {sc1.events(:).desc} ) );
