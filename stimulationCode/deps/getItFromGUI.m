function output = getItFromGUI( ctrlLabels, ctrlValues, ctrlVariableNames, prefGroupName, guiTitle, setCallerVariables, hUpdater )
    ...
    if ~exist( 'setCallerVariables', 'var' ) || isempty( setCallerVariables ),
    ...
    useExternalUpdater = exist( 'hUpdater', 'var' ) && ~isempty( hUpdater ) && isa( hUpdater, 'function_handle'  );
    
    usePrefs = (nargin > 2) && exist( 'prefGroupName', 'var' ) && ~isempty( prefGroupName ) && ischar( prefGroupName ); % && ispref( prefGroupName );
    if usePrefs,
        prefs = getpref( prefGroupName );
    end
    ...
    nParCols            = ceil( nControls / 40 );
    nParRows            = ceil( nControls / nParCols );
    ...
    columnWidth         = 2*labelWidth + 3*marginH;
    dialogWidth         = nParCols*columnWidth;
    dialogHeight        = marginV + nParRows*(ctrlHeight+marginV) + startButtonHeight + marginV;
    ...
    for iC = 1:nControls,
        iCol = ceil( iC / nParRows ) - 1;
        iRow = iC - iCol*nParRows;
        horOffset = marginH + iCol*columnWidth;

        if usePrefs,
            defaultValue = myGetPref( prefNames{iC}, ctrlValues{iC} );
        else
        ...
        switch ctrlDescription.class,
            case 'cell', % pop-up menu
                ...
                uicontrol( hDialog, 'Style', 'text', ...
                    ...
                    'Position', [horOffset dialogHeight-iRow*(ctrlHeight+marginV) labelWidth ctrlHeight]);
                ...
                hCtrlList(iC) = uicontrol( hDialog, 'Style', 'popupmenu',...
                    ...
                    'Position', [horOffset+marginH+labelWidth dialogHeight-iRow*(ctrlHeight+marginV) labelWidth ctrlHeight], ...
                    'Callback', {@uiCallback}, ...
            ...
            case {'char', 'double'},
                if isnumeric( defaultValue ),
                    defaultValue = mat2str( defaultValue );  % num2str
                end
                % ctrl's label (static text)
                uicontrol( hDialog, 'Style','text', ...
                    'String', [ctrlLabels{iC} ':'] , ...
            ...
            case 'logical',
                ...
                hCtrlList(iC) = uicontrol( hDialog, 'Style', 'checkbox',...
                    ...
                    'HorizontalAlignment', 'right', ...
                    'BackgroundColor', color, ...
                    'Position', [round(dialogWidth/2-labelWidth/2) dialogHeight-iRow*(ctrlHeight+marginV) labelWidth ctrlHeight],  ...
                    'Callback', {@uiCallback}, ...
                    'UserData', ctrlDescription );
            ...
            case 'struct',
                ...
                hCtrlList(iC) = uicontrol( hDialog, 'Style', 'pushbutton',...
                    ...
                    'BackgroundColor', color, ...
                    'Position', [horOffset dialogHeight-iRow*(ctrlHeight+marginV) 2*labelWidth+marginH ctrlHeight],  ...
    ...
    uicontrol( hDialog, 'Style', 'pushbutton', ...
        'String', 'Continue', ...
        'Position', [marginH marginV dialogWidth-2*marginH startButtonHeight], ...
        'BackgroundColor', color, ...
    ...
    function uiCallback( hObject, ~, subGUIdata )
        ...
        if( useExternalUpdater )
            [hObject, hCtrlList] = hUpdater( hObject, hCtrlList );
        end
        ...
        switch get( hObject, 'Style' ),
            ...
            case 'edit',
                ...
                switch ctrlInfo.class,
                    case {'double'},                        
                        output{ctrlInfo.index} = eval( ['[' value ']'] );
                    otherwise
    ...
    %---------------------------------------------------------------
    function val = myGetPref( prefName, defaultVal )
        if nargin < 2,
            defaultVal = [];
        end
        if usePrefs && isfield( prefs, prefName ),
            val = prefs.(prefName);
        else
            val = defaultVal;
        end
